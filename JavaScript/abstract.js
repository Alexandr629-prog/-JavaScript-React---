...
// УРОК 35(ASINC, DEFER, ДИНАМИЧЕСКИЕ СКРИПТЫ)!!

//<script deffer src="script.js"></script>-script загружается в фновом режим, 
//такие скрипты НИКОГДА не блокируют страницу!!!
//скрипты с таким атрибутом всегда выполняется когда дом-дерево уже готово
//они срабатывают до события DOMContentLoaded!!!
//такие скрипты сохранют последовательность относительно друг друга
// т.е. если первый скрипт большой, а второй маленький, то второй будет ждать
//пример: сначала подключаем jquery, а потм скрипт который работает с ним


//<script async src="script.js"></script> 
//1)  страница не ждем асинхронных скриптов, содержимое обрабатывается и отображается
//2)  DOMContentLoaded и асинхронные скрипты не ждут друг друга. т.е. скрипты не зависят друг от друга
// т.е при использование скрипотв с таким атрибутом мы должны быть точно уверены что дом-мтруктура загрузилась


const script =document.createElement('p');
script.src='js/test.js';
script.src=false;
document.body.append(script);//скрипт начнет выпооняться только после выполнения этой строчки
//динамически загружаемые скрипты по defoultu ведут себя как async
